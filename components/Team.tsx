...
export default function Team( ) {
    ...
    useEffect(() => {
        async function getTeamData() {
            const { data: teamdata, error } = await client.query({
                query: gql`
                query teams {
                    teams {
                      name
    ...
    return (
        <>
            {/* main div  */}
            <div className="max-w-7xl mx-auto px-5 md:px-0 py-10">
                {/* content div */}
                <div>
                    <div className="justify-center text-center">
                        <span className={`text-5xl ${myFont.className}`}>{`Team`}</span>
                    </div>
                    <div className="justify-center text-center py-6 ">
                        <span className={`${techsans.className} text-xl`}>{`Lorem ipsum dolor sit, amet consectetur adipisicing elit. Fugiat debitis aperiam rem id earum accusamus incidunt. Consequuntur dolor eaque alias. Ducimus, fugit ratione dolore aliquid dolorem ea unde sint nobis explicabo, facilis, soluta commodi.`}</span>
                    </div>
                </div>
                {/* content div ends*/}
                {loading ? (
                            ...
                            <>
                                <div className="grid lg:grid-cols-3 md:p-5 gap-5">
                                    ...
                                    {data.map((val: any, index: any) => {
                                        return (
                                            <>
                                                <div className="p-5  md:px-5 h-[400px] " key={val.name}>
                                                    <div className={`group relative block bg-black ${index % 3 == 1 ? "lg:mt-10" : ""} h-[400px]  `}>
                                                        <Image
                                                            width={200}
                                                            height={200}
                                                            alt="Profile Image"
                                                        ...
                                                        <div className="relative p-2 sm:p-2 lg:px-4 lg:py-4 overflow-hidden ">
                                                            <div className=" border-2 border-t-transparent border-r-transparent 
                                                            border-b-transparent md:border-0 group-hover:border-2 group-hover:border-r-transparent z-10 ">
                                                                ...
                                                                <div className="absolute  top-0 md:top-3 z-20   left-7   ">
                                                                    <p className={` ${manrope.className} px-1 text-sm  font-medium uppercase tlracking-widest text-lightblue`} border-r-transparent>
                                                                        {val.designation}
                                                                    </p>

                                                                    <p className={`text-base  text-white ${myFont.className}`}>{val.name}</p>
                                                                </div>
                                                                <div className="mt-32  p-4 h-[250px] md:h-[240px]">
                                                                    <div
                                                                        className="absolute bottom-7 left-3 translate-y-0 md:translate-y-8 transform opacity-100 md:opacity-0 transition-all group-hover:translate-y-0 group-hover:opacity-100"
                                                                    >
                                                                        <p className={`text-sm mt-12 p-1 md:p-3 text-white ${techsans.className}`}>
